...
paketinstall () {
	...
	echo -e "     \033[47;1;37m└\033[22;30m────────────────────────────────────────────────────────┘\033[44m"
	echo -e "\n\c"
  yast --remove patterns-openSUSE-minimal_base-11.4-6.9.1.x86_64
	yast -i apache2 apache2-mod_perl apache2-mod_php5 automake bison cairo dialog dejavu dos2unix flex fltk fping freetype gcc gcc-c++ gd gd-devel giflib glibc gmp-devel gnuplot gnutls gnutls-devel gtk2 joe libmcrypt libmcrypt-devel libmysqlclient-devel libpcap libpng-devel mailx make mc mysql-community-server libmysqld-devel libmysqlclient16 net-snmp net-snmp-devel nmap openldap2-client openldap2-devel openssl-devel perl-Algorithm-Diff perl-DBD-mysql perl-Net-DNS perl-Net-Daemon perl-Net-IP perl-Net-SNMP perl-Net-Server perl-Net-Telnet perl-SNMP perl-XML-LibXML perl-XML-RegExp php5 php5-devel php5-dom php5-gd php5-gettext php5-iconv php5-ldap php5-mbstring php5-mcrypt php5-mysql php5-pear php5-snmp php5-sockets php5-xsl php5-xmlreader php5-xmlrpc php5-zlib rrdtool sysstat xorg-x11-font zlib
	if [ $? != 0 ]; then
