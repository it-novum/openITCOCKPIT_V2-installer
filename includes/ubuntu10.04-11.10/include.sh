...
paketinstall () {
	...
	apt-get -y install apache2 libapache2-mod-perl2 libapache2-mod-php5 libapache2-mod-python automake bison libcairo2 libcairo2-dev dialog flex libfltk1.1 fping  libfreeradius2 libfreetype6 build-essential libgd2-xpm libgd2-xpm-dev libpango1.0-dev libxml2-dev libgif4 gnuplot libgnutls26 gnutls-bin joe php5 php5-dev php5-cgi php5-cli php-date php5-gd php-gettext php5-ldap php5-mcrypt php5-mysql php-pear php5-snmp php5-sqlite php5-xsl php5-dev php-xml-parser php5-xmlrpc php-xml-serializer sudo sysstat zlib1g zlib1g-dev mc mrtg libpng12-0 libpng12-dev mysql-server libmysqlclient15-dev libpcap0.8 libpcap0.8-dev libmcrypt4 libmcrypt-dev nmap mc snmp snmpd libsnmp15 libsnmp-dev libldap-2.4-2 libldap2-dev libssl-dev  libssl0.9.8 libalgorithm-diff-perl libdbd-mysql-perl libnet-dns-perl libnet-daemon-perl libnet-ip-perl libnet-snmp-perl libnet-server-perl libnet-telnet-perl libsnmp-perl libxml-libxml-perl libxml-regexp-perl vim librrds-perl librrdp-perl rrdtool
	if [ $? != 0 ]; then
		unimsg "ERR: A problem was detected by the execution of apt-get"
	fi
	echo -e "\033[0m"
...
sys_user_add () {
	...
	echo 'alias itc-test="/opt/openitc/nagios/bin/nagios -v /opt/openitc/nagios/etc/nagios.cfg"' >> /root/.bashrc
	echo 'alias itc="cd /opt/openitc/nagios"' >> /root/.bashrc
	echo 'export PS1="[\A]\[\e[31m\]\u\[\e[35m\]@\h\[\e[32m\]\w\[\e[0m\]# "' >> /root/.bashrc
	echo 'alias maint="crm configure property maintenance-mode=true"' >> /root/.bashrc
	echo 'alias unmaint="crm configure property maintenance-mode=false"' >> /root/.bashrc
